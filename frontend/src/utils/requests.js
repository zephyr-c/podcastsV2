import {readQuery, createQuery, updateQuery, discover, deleteQuery} from './api.js'
import { actionAddPodcast, actionLikePodcast, actionDislikePodcast, actionDeletePodcast } from './actions.js'
...
export const deletePodcast = (podcastId, dispatch) => {
    return deleteQuery({id: podcastId})
    .then((response) => {
        if(response.status === 204)
        dispatch(actionDeletePodcast(podcastId))
    })
}
